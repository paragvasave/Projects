...
void man(char *name)
{
    ...
    {
        printf("Desctiption : Used to change file offset.\n");
        printf("Usage : lseek File_Name ChangeInOffset StartPoint\n");
    }
    else if(strcmp(name,"rm") == 0)
    {
        printf("Desctiption : Used to delete the file\n");
...
int CreateFile(char *name,int permission)   //Arr[]
{
    int i = 3;
    PINODE temp = head; 

    if((name == NULL) || (permission == 0) || (permission > 3))     //Filter
        return -1;
    
    if(SUPERBLOCKobj.FreeInode == 0)        //iNode nai a so file create nai karu shakat
        return -2;
    
    (SUPERBLOCKobj.FreeInode)--;            //reduction of iNode

    if(Get_Inode(name) != NULL)             //Checking the name
        return -3;

    while(temp != NULL)
    {
        if(temp -> FileType == 0)           //Regular 1 Special 2       FileType vacant inode
            break;
        temp = temp -> next;
    }
    ...
    strcpy(UFDTArr[i].ptrfiletable -> ptrinode -> FileName,name);       //File ch name copy zal
    ...
    while(1)
    {
        ...
        else if(count == 2)
        {
            ...
            else if(strcmp(command[0],"truncate") == 0)
            {
                ...
                if(ret == -1)
                    printf("ERROR : Incorrect paramet.\n");
            }
            else
            {
                printf("\nERROR : Command not found...!\n");
